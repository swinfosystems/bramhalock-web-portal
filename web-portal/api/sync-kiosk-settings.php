...
try {
    ...
    // Check if kiosk settings exist
    $stmt = $pdo->prepare("SELECT id FROM system_settings WHERE device_id = ? AND setting_key = 'kiosk_settings'");
    $stmt->execute([$deviceDbId]);
    ...
    if ($existingSettings) {
        ...
        $stmt = $pdo->prepare("
            UPDATE system_settings SET 
                setting_value = ?,
                updated_at = NOW()
            WHERE device_id = ? AND setting_key = 'kiosk_settings'
        ");
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, 'kiosk_settings', ?, NOW(), NOW())
        ");
        $stmt->execute([$deviceDbId, $settingsJson]);
    }
    
    // Log the kiosk settings update
    logSecurityEvent($deviceDbId, 'kiosk_settings_updated', [
        'kiosk_mode' => $settings['kioskMode'],
        'allowed_apps_count' => count($settings['allowedApps']),
        'restricted_websites_count' => count($settings['restrictedWebsites']),
        'auto_lock_timeout' => $settings['autoLockTimeout']
    ]);
    
    echo json_encode([
        'success' => true,
        'message' => 'Kiosk settings synced successfully',
        'device_id' => $deviceDbId
    ]);
    
} catch (Exception $e) {
    error_log('Kiosk settings sync error: ' . $e->getMessage());
